...
public interface BulletinRepository extends BulletinRepositoryWithBagRelationships, JpaRepository<Bulletin, Long> {
    ...
    @Query(
        value = "select distinct bulletin from Bulletin bulletin left join fetch bulletin.etudiant",
    ...
    @Query("select distinct bulletin from Bulletin bulletin left join fetch bulletin.etudiant")
    List<Bulletin> findAllWithToOneRelationships();

    @Query("select bulletin from Bulletin bulletin left join fetch bulletin.etudiant where bulletin.id =:id")
