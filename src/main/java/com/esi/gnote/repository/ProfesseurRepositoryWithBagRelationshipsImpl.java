...
public class ProfesseurRepositoryWithBagRelationshipsImpl implements ProfesseurRepositoryWithBagRelationships {
    ...
    public Optional<Professeur> fetchBagRelationships(Optional<Professeur> professeur) {
        return professeur.map(this::fetchEtudiants).map(this::fetchClasses).map(this::fetchECS);
    }
    ...
    Professeur fetchECS(Professeur result) {
        return entityManager
            .createQuery(
                "select professeur from Professeur professeur left join fetch professeur.eCS where professeur is :professeur",
                Professeur.class
            )
            .setParameter("professeur", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
    ...
    List<Professeur> fetchECS(List<Professeur> professeurs) {
        return entityManager
            .createQuery(
                "select distinct professeur from Professeur professeur left join fetch professeur.eCS where professeur in :professeurs",
                Professeur.class
            )
            .setParameter("professeurs", professeurs)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
