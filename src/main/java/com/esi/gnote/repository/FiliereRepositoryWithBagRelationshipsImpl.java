...
public class FiliereRepositoryWithBagRelationshipsImpl implements FiliereRepositoryWithBagRelationships {
    ...
    public Optional<Filiere> fetchBagRelationships(Optional<Filiere> filiere) {
        return filiere.map(this::fetchUES);
    }
    ...
    public List<Filiere> fetchBagRelationships(List<Filiere> filieres) {
        return Optional.of(filieres).map(this::fetchUES).orElse(Collections.emptyList());
    }

    Filiere fetchUES(Filiere result) {
        return entityManager
            .createQuery("select filiere from Filiere filiere left join fetch filiere.uES where filiere is :filiere", Filiere.class)
            .setParameter("filiere", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Filiere> fetchUES(List<Filiere> filieres) {
        return entityManager
            .createQuery(
                "select distinct filiere from Filiere filiere left join fetch filiere.uES where filiere in :filieres",
                Filiere.class
            )
