...
public class NoteRepositoryWithBagRelationshipsImpl implements NoteRepositoryWithBagRelationships {
    ...
    public Optional<Note> fetchBagRelationships(Optional<Note> note) {
        return note.map(this::fetchEtudiants).map(this::fetchECS);
    }
    ...
    public List<Note> fetchBagRelationships(List<Note> notes) {
        return Optional.of(notes).map(this::fetchEtudiants).map(this::fetchECS).orElse(Collections.emptyList());
    }
    ...
    Note fetchECS(Note result) {
        return entityManager
            .createQuery("select note from Note note left join fetch note.eCS where note is :note", Note.class)
            .setParameter("note", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Note> fetchECS(List<Note> notes) {
        return entityManager
            .createQuery("select distinct note from Note note left join fetch note.eCS where note in :notes", Note.class)
            .setParameter("notes", notes)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getResultList();
    }
