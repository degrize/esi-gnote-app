...
public class ClasseRepositoryWithBagRelationshipsImpl implements ClasseRepositoryWithBagRelationships {
    ...
    public Optional<Classe> fetchBagRelationships(Optional<Classe> classe) {
        return classe.map(this::fetchECS);
    }
    ...
    public List<Classe> fetchBagRelationships(List<Classe> classes) {
        return Optional.of(classes).map(this::fetchECS).orElse(Collections.emptyList());
    }

    Classe fetchECS(Classe result) {
        return entityManager
            .createQuery("select classe from Classe classe left join fetch classe.eCS where classe is :classe", Classe.class)
            .setParameter("classe", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Classe> fetchECS(List<Classe> classes) {
        return entityManager
            .createQuery("select distinct classe from Classe classe left join fetch classe.eCS where classe in :classes", Classe.class)
